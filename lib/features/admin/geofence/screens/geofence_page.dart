...
class _GeofencePageState extends State<GeofencePage> {
  ...
  Widget build(BuildContext context) {
    final markers = <Marker>[
      ..._geofences.map((geo) {
        final lat =
            double.tryParse(geo['center_lat']?.toString() ?? '0') ?? 0.0;
        final lng =
            double.tryParse(geo['center_lng']?.toString() ?? '0') ?? 0.0;
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return ListTile(
                          ...
                          onTap: () =>
                              _moveToLocation(result['lat'], result['lon']),
                        );
                      },
                    ),
                  ),
                        ...
                        CircleLayer(
                          circles: _geofences.map((geo) {
                            final lat = double.tryParse(
                                    geo['center_lat']?.toString() ?? '0') ??
                                0.0;
                            final lng = double.tryParse(
                                    geo['center_lng']?.toString() ?? '0') ??
                                0.0;
                            final radius = double.tryParse(
                                    geo['radius']?.toString() ?? '100') ??
                                100.0;
                            final type = geo['type'] ?? 'office';
                            final color = _getGeofenceColor(type);
                            return CircleMarker(
